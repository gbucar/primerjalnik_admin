...
<div class="flex flex-col items-center p-3">
	<div class="bg-white mb-3 rounded-3xl shadow-xl p-3 sm:w-5/6 w-full transition-all">
		<h2 class="font-bold text-3xl">Povprečni vnos delcev</h2>
		{#if originalRecords + alternativeRecords}
			<div class="text-center w-full block text-6xl font-bold">
				{formatIntakeNg(
					(originalIntake * originalRecords + alternativeIntake * alternativeRecords) /
						(originalRecords + alternativeRecords)
				)}
			</div>
			<small class="text-center w-full block"
				>Povprečen vnos delcev vseh poti (alternativnih in originalnih)</small
			>
		{:else}
			<h>Na voljo ni nobenih meritev</h>
		{/if}
	</div>

	<div class="bg-white mb-3 rounded-3xl shadow-xl p-3 sm:w-5/6 w-full transition-all">
		<h2 class="font-bold text-3xl">Vizualizacija s krogi</h2>
		{#if originalRecords > 2 && alternativeRecords > 2}
			...
			<small class="text-center w-full block">
				Kroga prikazujeta povprečen vnos delcev (PM1) za originalno (levi krog) in alternativno
				(desni krog) pot v nanogramih (µg) delcev na kilogram (kg) telesne teže. Večji kot je krog,
				večji je povprečen vnos delcev. V kolikor menite, da je rezultat napačen preverite, če so
				posnetki v zavihku "Zgodovina" pravilno označeni. <br />
				Opozorilo: pri ekstremnih vrednostih razmerja niso točna.
			</small>
		{:else}
			<h>Za prikaz sta potrebni vsaj dve alternativni in dve originalni poti</h>
		{/if}
	</div>

	<div class="bg-white p-3 rounded-3xl shadow-xl sm:w-5/6 w-full transition-all">
		<h2 class="font-bold text-3xl">Graf vseh posnetih poti (vseh udeležencev)</h2>
		<div class="py-2">
